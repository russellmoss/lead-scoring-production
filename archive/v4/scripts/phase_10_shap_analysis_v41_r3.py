"""
Phase 10: SHAP Analysis for V4.1 R3
...
def evaluate_gates(explainer_created, shap_calculated, shap_importance, xgb_importance, feature_names):
    ...
    return gates, top_10_features if shap_importance is not None else None
...
"""
    ...
    if shap_rankings is not None:
        ...
        for idx, row in shap_rankings.head(10).iterrows():
            ...
            report += f"| {len(shap_rankings) - shap_rankings.index.get_loc(idx)} | {row['feature']} | {row['shap_importance']:.4f} |{is_new}\n"
...
"""
    ...
    if gates['G10.3']:
        report += "- ✅ New V4.1 features are well-represented in top SHAP features\n"
    else:
        report += "- ⚠️ New V4.1 features may need further investigation\n"
    
    if gates['G10.4']:
        report += "- ✅ SHAP and XGBoost importance are well-aligned (validates model consistency)\n"
    else:
        report += "- ⚠️ SHAP and XGBoost importance show some divergence (investigate)\n"
    
    report += f"""
## Conclusion

{'✅ All validation gates passed. SHAP analysis confirms model interpretability and feature importance.' if all(gates.values()) else '⚠️ Some validation gates failed. Review above for details.'}
"""
...
def run_phase_10():
    ...
    print("=" * 80)
    print("Phase 10: SHAP Analysis - V4.1 R3")
    print("=" * 80)
