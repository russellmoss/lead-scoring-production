"""
Phase 8 Revision 2: Overfitting Detection for V4.1 R2 Model
...
def load_model_and_data():
    """Load V4.1 R2 model and train/test data."""
...
def run_cross_validation(X, y, n_splits=5):
    ...
    model = XGBClassifier(
        max_depth=3,  # R2 hyperparameters
        min_child_weight=20,
        gamma=0.2,
        subsample=0.7,
...
def generate_report(metrics, gates_results, metrics_r1=None):
    """Generate overfitting report."""
    print("\n[5/5] Generating report...")
    
    report_path = REPORT_DIR / "overfitting_report_r2.md"
    results_path = REPORT_DIR / "overfitting_results_r2.json"
...
|--------|----|----|--------|--------|
| Test AUC | {metrics_r1['test_auc']:.4f} | {metrics['test_auc']:.4f} | {metrics['test_auc'] - metrics_r1['test_auc']:+.4f} | {'IMPROVED' if metrics['test_auc'] > metrics_r1['test_auc'] else 'WORSE'} |
| AUC Gap | {metrics_r1['auc_gap']:.4f} | {metrics['auc_gap']:.4f} | {metrics['auc_gap'] - metrics_r1['auc_gap']:+.4f} | {'IMPROVED' if metrics['auc_gap'] < metrics_r1['auc_gap'] else 'WORSE'} |
| Test Top Decile Lift | {metrics_r1['test_top_lift']:.2f}x | {metrics['test_top_lift']:.2f}x | {metrics['test_top_lift'] - metrics_r1['test_top_lift']:+.2f}x | {'IMPROVED' if metrics['test_top_lift'] > metrics_r1['test_top_lift'] else 'WORSE'} |
| Lift Gap | {metrics_r1['lift_gap']:.2f}x | {metrics['lift_gap']:.2f}x | {metrics['lift_gap'] - metrics_r1['lift_gap']:+.2f}x | {'IMPROVED' if metrics['lift_gap'] < metrics_r1['lift_gap'] else 'WORSE'} |
| CV Mean AUC | {metrics_r1['cv_mean']:.4f} | {metrics['cv_mean']:.4f} | {metrics['cv_mean'] - metrics_r1['cv_mean']:+.4f} | {'IMPROVED' if metrics['cv_mean'] > metrics_r1['cv_mean'] else 'WORSE'} |
"""
    
    report = f"""# V4.1 R2 Overfitting Detection Report
...
{'**WARNING**: Large AUC gap indicates significant overfitting. Model is memorizing training patterns.' if not gates_results['G8.1'] else ''}
...
{'**WARNING**: Test AUC is below threshold and below V4.0.0 baseline. Model may not be ready for deployment.' if not gates_results['G8.4'] else ''}
...
"""
    ...
    if not gates_results['G8.1']:
        report += """### Overfitting Still Detected (G8.1 Failed)

**Issue**: Large train-test AUC gap ({:.4f}) indicates significant overfitting persists.
...
{'PASSED: No overfitting detected. Model shows good generalization to test set.' if all_passed else 'FAILED: Overfitting detected. Review recommendations above and consider retraining with adjusted hyperparameters.'}
"""
...
def run_phase_8_r2():
    """Execute Phase 8 R2: Overfitting Detection."""
    print("=" * 80)
    print("Phase 8 Revision 2: Overfitting Detection - V4.1 R2")
    print("=" * 80)
    ...
    try:
        ...
        # Generate report
        all_passed = generate_report(metrics, gates_results, metrics_r1)
        
        print("\n" + "=" * 80)
        print("Phase 8 R2 Complete!")
        print("=" * 80)
