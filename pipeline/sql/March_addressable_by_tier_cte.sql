CREATE OR REPLACE TABLE `savvy-gtm-analytics.ml_features.march_addressable_by_tier` AS
WITH 
-- ============================================================================
...
career_clock_stats AS (
    ...
    WHERE eh.PREVIOUS_REGISTRATION_COMPANY_END_DATE IS NOT NULL
      ...
      -- âš ï¸ PIT CRITICAL: Only completed jobs BEFORE CURRENT_DATE
...
SELECT final_tier AS score_tier, COUNT(*) AS addressable_count
FROM deduplicated_before_quotas
GROUP BY final_tier
ORDER BY addressable_count DESC;
